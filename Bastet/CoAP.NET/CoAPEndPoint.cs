...
namespace CoAP.Net
{
    ...
    public class CoAPEndPoint : IEndPoint, IExchangeForwarder
    {
        ...
        {
            Task.Factory.StartNew(() => _coapStack.SendRequest(request));
        }

        /// <inheritdoc/>
        public void SendResponse(Exchange exchange, Response response)
        {
            Task.Factory.StartNew(() => _coapStack.SendResponse(exchange, response));
        }

        /// <inheritdoc/>
        public void SendEmptyMessage(Exchange exchange, EmptyMessage message)
        {
            Task.Factory.StartNew(() => _coapStack.SendEmptyMessage(exchange, message));
        }

        private void ReceiveData(Object sender, DataReceivedEventArgs e)
        {
            Task.Factory.StartNew(() => ReceiveData(e));
        }
